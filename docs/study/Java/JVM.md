...
## JVM 内存
  ...
  JVM 内存区域按照运行状态划分：运行时数据区域和非运行时数据区域；JDK 1.8前后内存区域分布不同。

  JVM 内存区域按照线程状态划分：线程共享和线程私有
...
#### 堆
  ...
  - `新生代内存`(Young Generation)：Eden、From Survivor、To Survivor
  - `老生代`(Old Generation)
  - `永生代`(Permanent Generation)

  JDK 8 版本之后方法区（HotSpot 的永久代）被彻底移除了（JDK1.7 就已经开始了），取而代之是`元空间`，`元空间`使用的是直接内存。

  新生代包含 Eden 区、两个 Survivor 区（为了区分，这两个 Survivor 区域按照顺序被命名为 from 和 to），中间一层属于老年代。
...
#### 程序计数器
  ...
  单线程角度(?)：字节码解释器工作时通过改变这个计数器的值来选取下一条需要执行的字节码指令，
  包括分支、循环、跳转、异常处理、线程恢复等功能都需要依赖这个计数器来完成。

  多线程角度(?)：为了线程切换后能恢复到正确的执行位置，每条线程都需要有一个独立的程序计数器，
...
> Java 内存可以粗糙的区分为堆内存（Heap）和栈内存 (Stack),其中栈就是现在说的虚拟机栈，或者说是虚拟机栈中局部变量表部分。 （实际上，Java 虚拟机栈是由一个个`栈帧`组成，而每个`栈帧`中都拥有：局部变量表、操作数栈、动态链接、方法出口信息。）

  局部变量表主要存放了编译器可知的各种`基本数据类型`（boolean、byte、char、short、int、float、long、double）、对象引用（reference 类型，它不同于对象本身，可能是一个指向对象起始地址的引用指针，也可能是指向一个代表对象的`句柄`或其他与此对象相关的位置）。
...
#### 设置对象头
  ...
  初始化零值完成之后，虚拟机要对对象进行必要的设置，例如这个对象是那个类的实例、如何才能找到类的元数据信息、对象的哈希码、对象的 GC 分代年龄等信息。 这些信息存放在 `对象头` 中。
...
### 对象访问定位
  ...
  对象的访问方式由虚拟机实现而定，目前主流的访问方式有使用 `句柄` 和 `直接指针` 两种：
